...
function TranslateFlow({ onClose }: { onClose: () => void }) {
  ...
  return <ToolSheet title="Translator" subtitle="English → Japanese, works offline" onClose={onClose}>
    ...
    <div className="tool-actions"><button className="scan-chip" disabled={!text.trim()} onClick={() => run(text)}><Languages size={17} /> Translate</button><button className="scan-chip" onClick={() => run("Does this have meat or fish?")}><Camera size={17} /> Scan menu</button></div>
