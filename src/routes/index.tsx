    ...
    <div className="spend-timeline">{days.map((d) => <section key={d}>
      ...
      {expenses.filter((e) => e.day === d).sort((a, b) => a.time.localeCompare(b.time)).map((e) => {
        ...
        return <article key={e.id}>
          ...
          <p className="cost-place">{stop ? `${stop.title} · ${stop.place}` : e.place || "No activity matched"}</p>
          <p className="cost-foot"><span className="split-tag"><Users size={13} /> Split {splitLabel(e.split)}</span><small>{e.source === "scan" ? "Receipt" : "Manual"} · {e.payer}</small></p>
        </article>;
      })}
    </section>)}</div>
